...
export function CrearProyecto (props) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      console.log(data)
      const response = await axios.post(`${config.URL}proyecto`, data)
      setIsOpen(true)
      return (response)
    } catch (error) {
  ...
  return (
        ...
        <AlertDialogOverlay>
            < AlertDialogContent>
                <Spacer y={1} />
                <AlertDialogBody className='text-2xl text-center'>Proyecto creado satisfactoriamente </AlertDialogBody>
                <AlertDialogFooter>
